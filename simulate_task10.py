...
def jacobi_gpu(u, interior_mask, max_iter, atol=1e-6, check_every=500):
    ...
    Whole-array CuPy ops only (Python-level element indexing would launch
    one kernel per cell and never finish). `cp.where` replaces boolean-mask
    indexing because the latter has data-dependent output size and forces
    a host sync to allocate.
    """
    ...
    for it in range(max_iter):
        u_new = 0.25 * (u[1:-1, :-2] + u[1:-1, 2:]
                        + u[:-2, 1:-1] + u[2:, 1:-1])
        u_int = u[1:-1, 1:-1]
        u_int_new = cp.where(mask, u_new, u_int)
        if it % check_every == 0:
            delta = cp.abs(cp.where(mask, u_int - u_new, 0.0)).max()
            if float(delta) < atol:
                u[1:-1, 1:-1] = u_int_new
                break
        u[1:-1, 1:-1] = u_int_new
